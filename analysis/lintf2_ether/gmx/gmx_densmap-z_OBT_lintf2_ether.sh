...
#SBATCH --job-name="densmap-z_OBT"             # Name for the job allocation
#SBATCH --output="densmap-z_OBT_slurm-%j.out"  # Filename to which standard output and error will be written
...
saveDir="densmap-z_OBT_${zmin}-${zmax}nm_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_densmap-z_OBT_${zmin}-${zmax}nm.dat\
