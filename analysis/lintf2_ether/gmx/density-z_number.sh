...
#SBATCH --job-name="gmx_density-z_number"
#SBATCH --output="gmx_density-z_number_slurm-%j.out"
#SBATCH --nodes=1
...
echo "${selection}" |
    ${gmx_exe} density \
        ...
        -n "${system}.ndx" \
        -o "${settings}_${system}_density-z_number.xvg" \
        -b "${begin}" \
        -e "${end}" \
        -dt "${dt}" \
        -d Z \
...
save_dir="density-z_number_slurm-${SLURM_JOB_ID}"
if [[ ! -d ${save_dir} ]]; then
    echo -e "\n"
    mkdir -v "${save_dir}" || exit
    mv -v \
        "${settings}_${system}_density-z_number.xvg" \
        "${settings}_${system}_density-z_number_slurm-${SLURM_JOB_ID}.out" \
        "${save_dir}"
