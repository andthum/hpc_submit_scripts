...
#SBATCH --job-name="potential-z"             # Name for the job allocation
#SBATCH --output="potential-z_slurm-%j.out"  # Filename to which standard output and error will be written
...
echo "=================================================================="
echo 0\
    | gmx potential -f ${settings}_out_${system}_pbc_whole_mol.xtc\
...
saveDir="potential-z_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_potential-z.xvg\
