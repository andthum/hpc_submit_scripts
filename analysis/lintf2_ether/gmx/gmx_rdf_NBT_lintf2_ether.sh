...
#SBATCH --job-name="rdf_NBT"             # Name for the job allocation
#SBATCH --output="rdf_NBT_slurm-%j.out"  # Filename to which standard output and error will be written
...
gmx rdf -f ${settings}_out_${system}_pbc_whole_mol.xtc\
        ...
        -o ${settings}_${system}_rdf_NBT.xvg\
        -cn ${settings}_${system}_cnrdf_NBT.xvg\
        -b ${begin}\
        -e ${end}\
        -dt ${dt}\
        -bin ${bin_width}\
...
saveDir="rdf_NBT_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_rdf_NBT.xvg\
          ${settings}_${system}_cnrdf_NBT.xvg\
          ${settings}_${system}_rdf_NBT_slurm-${SLURM_JOB_ID}.out\
          ${saveDir}\
        || exit
