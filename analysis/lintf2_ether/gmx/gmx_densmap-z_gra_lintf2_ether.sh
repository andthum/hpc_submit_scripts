...
#SBATCH --job-name="densmap-z_gra"             # Name for the job allocation
#SBATCH --output="densmap-z_gra_slurm-%j.out"  # Filename to which standard output and error will be written
...
bin_width=${6}  # Grid size in the investigated plane in nm
...
if [[ ${system} != *gra* ]]; then
    echo
    echo "Error: The system contains no graphene electrodes."
    exit 1
fi
echo -e "\n\n\n"
echo "Graphene slab-z ${zmin}-${zmax} nm"
echo
echo "=================================================================="
echo 23 \
    | gmx densmap -f ${settings}_out_${system}.trr\
                  -s ${settings}_${system}.tpr\
                  -n ${system}.ndx\
                  -od ${settings}_${system}_densmap-z_gra_${zmin}-${zmax}nm.dat\
                  -b ${begin}\
                  -e ${end}\
                  -dt ${dt}\
                  -bin ${bin_width}\
...
saveDir="densmap-z_gra_${zmin}-${zmax}nm_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_densmap-z_gra_${zmin}-${zmax}nm.dat\
          ${settings}_${system}_densmap-z_gra_${zmin}-${zmax}nm_slurm-${SLURM_JOB_ID}.out\
          ${saveDir}\
        || exit
