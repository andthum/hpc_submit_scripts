...
#SBATCH --job-name="gmx_trjconv_whole"
#SBATCH --output="gmx_trjconv_whole_slurm-%j.out"
#SBATCH --nodes=1
...
echo "System" |
    ${gmx_exe} trjconv \
        -f "${settings}_out_${system}_pbc_whole.xtc" \
        -s "${settings}_${system}.tpr" \
...
save_dir="trjconv_whole_slurm-${SLURM_JOB_ID}"
if [[ ! -d ${save_dir} ]]; then
    echo -e "\n"
    mkdir -v "${save_dir}" || exit
    mv -v \
        "${settings}_${system}_trjconv_whole_slurm-${SLURM_JOB_ID}.out" \
        "${save_dir}"
