...
#SBATCH --job-name="Mdens-z"             # Name for the job allocation
#SBATCH --output="Mdens-z_slurm-%j.out"  # Filename to which standard output and error will be written
...
echo ${selection}\
    | gmx density -f ${settings}_out_${system}_pbc_whole_mol.xtc\
                  ...
                  -n ${system}.ndx\
                  -o ${settings}_${system}_Mdens-z.xvg\
                  -b ${begin}\
                  -e ${end}\
                  -dt ${dt}\
                  -d Z\
...
saveDir="Mdens-z_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_Mdens-z.xvg\
          ${settings}_${system}_Mdens-z_slurm-${SLURM_JOB_ID}.out\
          ${saveDir}\
        || exit
