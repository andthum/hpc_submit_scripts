...
#SBATCH --job-name="rdf_slab-z_OE"             # Name for the job allocation
#SBATCH --output="rdf_slab-z_OE_slurm-%j.out"  # Filename to which standard output and error will be written
...
gmx rdf -f ${settings}_out_${system}_pbc_whole_mol.xtc\
        ...
        -o ${settings}_${system}_rdf_slab-z_OE_${zmin}-${zmax}nm.xvg\
        -cn ${settings}_${system}_cnrdf_slab-z_OE_${zmin}-${zmax}nm.xvg\
        -b ${begin}\
        -e ${end}\
        -dt ${dt}\
        -bin ${bin_width}\
...
saveDir="rdf_slab-z_OE_${zmin}-${zmax}nm_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_rdf_slab-z_OE_${zmin}-${zmax}nm.xvg\
          ${settings}_${system}_cnrdf_slab-z_OE_${zmin}-${zmax}nm.xvg\
          ${settings}_${system}_rdf_slab-z_OE_${zmin}-${zmax}nm_slurm-${SLURM_JOB_ID}.out\
          ${saveDir}\
        || exit
