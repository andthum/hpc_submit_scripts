...
#SBATCH --job-name="densmap-z_OE"             # Name for the job allocation
#SBATCH --output="densmap-z_OE_slurm-%j.out"  # Filename to which standard output and error will be written
...
if [[ ${system} == *gra* ]]; then
    selection="30"  # <ETHER>_&_O*
else
    selection="14"  # <ETHER>_&_O*
fi
echo -e "\n\n\n"
echo "OE slab-z ${zmin}-${zmax} nm"
echo
echo "=================================================================="
echo ${selection} \
    | gmx densmap -f ${settings}_out_${system}.trr\
                  ...
                  -n ${system}.ndx\
                  -od ${settings}_${system}_densmap-z_OE_${zmin}-${zmax}nm.dat\
                  -b ${begin}\
                  -e ${end}\
                  -dt ${dt}\
                  -bin ${bin_width}\
...
saveDir="densmap-z_OE_${zmin}-${zmax}nm_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_densmap-z_OE_${zmin}-${zmax}nm.dat\
          ${settings}_${system}_densmap-z_OE_${zmin}-${zmax}nm_slurm-${SLURM_JOB_ID}.out\
          ${saveDir}\
        || exit
