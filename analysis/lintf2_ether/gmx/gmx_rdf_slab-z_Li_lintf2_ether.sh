...
#SBATCH --job-name="rdf_slab-z_Li"             # Name for the job allocation
#SBATCH --output="rdf_slab-z_Li_slurm-%j.out"  # Filename to which standard output and error will be written
...
gmx rdf -f ${settings}_out_${system}_pbc_whole_mol.xtc\
        ...
        -bin ${bin_width}\
        -xy\
        -ref "atomtype Li and (z >= ${zmin} and z < ${zmax})"\
        -sel "atomtype Li and (z >= ${zmin} and z < ${zmax})"\
             "atomtype NBT and (z >= ${zmin} and z < ${zmax})"\
             "atomtype OBT and (z >= ${zmin} and z < ${zmax})"\
...
saveDir="rdf_slab-z_Li_${zmin}-${zmax}nm_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_rdf_slab-z_Li_${zmin}-${zmax}nm.xvg\
