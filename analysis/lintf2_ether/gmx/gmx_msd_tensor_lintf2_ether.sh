...
#SBATCH --job-name="msd_tensor"             # Name for the job allocation
#SBATCH --output="msd_tensor_slurm-%j.out"  # Filename to which standard output and error will be written
...
echo ${selection}\
    | gmx msd -f ${settings}_out_${system}_pbc_whole_mol_nojump.xtc\
              ...
              -n ${system}.ndx\
              -o ${settings}_${system}_msd_tensor.xvg\
              -b ${begin}\
              -e ${end}\
              -ten\
              -ngroup 4\
...
saveDir="msd_tensor_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_msd_tensor.xvg\
          ${settings}_${system}_msd_tensor_slurm-${SLURM_JOB_ID}.out\
          ${saveDir}\
        || exit
