...
#SBATCH --job-name="polystat"             # Name for the job allocation
#SBATCH --output="polystat_slurm-%j.out"  # Filename to which standard output and error will be written
...
echo "=================================================================="
echo "${selection}"\
    | gmx polystat -f ${settings}_out_${system}_pbc_whole_mol.xtc\
...
saveDir="polystat_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_polystat.xvg\
