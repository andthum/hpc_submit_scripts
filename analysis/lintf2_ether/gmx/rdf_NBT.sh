...
#SBATCH --job-name="gmx_rdf_NBT"
#SBATCH --output="gmx_rdf_NBT_slurm-%j.out"
#SBATCH --nodes=1
...
save_dir="rdf_NBT_slurm-${SLURM_JOB_ID}"
if [[ ! -d ${save_dir} ]]; then
    echo -e "\n"
    mkdir -v "${save_dir}" || exit
    mv -v \
        "${settings}_${system}_rdf_NBT.xvg" \
