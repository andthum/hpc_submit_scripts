...
#SBATCH --job-name="rdf_slab-z_NTf2-com"             # Name for the job allocation
#SBATCH --output="rdf_slab-z_NTf2-com_slurm-%j.out"  # Filename to which standard output and error will be written
...
echo -e "\n\n\n"
echo "${anion}-${anion} (COM), ${anion}-${solvent} (COM) slab-z ${zmin}-${zmax} nm"
echo
echo "=================================================================="
gmx rdf -f ${settings}_out_${system}_pbc_whole_mol.xtc\
        -s ${settings}_${system}.tpr\
        -o ${settings}_${system}_rdf_slab-z_NTf2-com_${zmin}-${zmax}nm.xvg\
        -cn ${settings}_${system}_cnrdf_slab-z_NTf2-com_${zmin}-${zmax}nm.xvg\
        -b ${begin}\
        -e ${end}\
        -dt ${dt}\
        -bin ${bin_width}\
        -xy\
        -ref "whole_res_com of resname ${anion} and (z >= ${zmin} and z < ${zmax})"\
...
saveDir="rdf_slab-z_NTf2-com_${zmin}-${zmax}nm_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_rdf_slab-z_NTf2-com_${zmin}-${zmax}nm.xvg\
          ${settings}_${system}_cnrdf_slab-z_NTf2-com_${zmin}-${zmax}nm.xvg\
          ${settings}_${system}_rdf_slab-z_NTf2-com_${zmin}-${zmax}nm_slurm-${SLURM_JOB_ID}.out\
          ${saveDir}\
        || exit
