...
#SBATCH --job-name="msd_parallel-z"             # Name for the job allocation
#SBATCH --output="msd_parallel-z_slurm-%j.out"  # Filename to which standard output and error will be written
...
saveDir="msd_parallel-z_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_msd_parallel-z.xvg\
