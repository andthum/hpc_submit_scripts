...
#SBATCH --job-name="msd"             # Name for the job allocation
#SBATCH --output="msd_slurm-%j.out"  # Filename to which standard output and error will be written
...
saveDir="msd_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_msd.xvg\
          ${settings}_${system}_msd_slurm-${SLURM_JOB_ID}.out\
          ${saveDir}\
        || exit
