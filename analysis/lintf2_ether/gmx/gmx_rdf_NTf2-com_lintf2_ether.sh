...
#SBATCH --time=2-00:00:00         # Total run time limit. The default time limit is the partition's default time limit. On bagheera: long 7-00:00:00, short 2-00:00:00, gpu 2-00:00:00, prio 0-01:00:00
...
#SBATCH --job-name="rdf_NTf2-com"             # Name for the job allocation
#SBATCH --output="rdf_NTf2-com_slurm-%j.out"  # Filename to which standard output and error will be written
...
echo -e "\n\n\n"
echo "${anion}-${anion} (COM), ${anion}-${solvent} (COM)"
echo
echo "=================================================================="
gmx rdf -f ${settings}_out_${system}_pbc_whole_mol.xtc\
        -s ${settings}_${system}.tpr\
        -o ${settings}_${system}_rdf_NTf2-com.xvg\
        -cn ${settings}_${system}_cnrdf_NTf2-com.xvg\
        -b ${begin}\
        -e ${end}\
        -dt ${dt}\
        -bin ${bin_width}\
        -ref "whole_res_com of resname ${anion}"\
        -sel "whole_res_com of resname ${anion}"\
             "whole_res_com of resname ${solvent}"\
    || exit
echo "=================================================================="
...
saveDir="rdf_NTf2-com_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_rdf_NTf2-com.xvg\
          ${settings}_${system}_cnrdf_NTf2-com.xvg\
          ${settings}_${system}_rdf_NTf2-com_slurm-${SLURM_JOB_ID}.out\
          ${saveDir}\
        || exit
