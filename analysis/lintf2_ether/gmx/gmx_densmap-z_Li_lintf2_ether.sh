...
#SBATCH --job-name="densmap-z_Li"             # Name for the job allocation
#SBATCH --output="densmap-z_Li_slurm-%j.out"  # Filename to which standard output and error will be written
...
echo -e "\n\n\n"
echo "Li slab-z ${zmin}-${zmax} nm"
echo
echo "=================================================================="
echo 2 \
    | gmx densmap -f ${settings}_out_${system}.trr\
                  -s ${settings}_${system}.tpr\
                  -od ${settings}_${system}_densmap-z_Li_${zmin}-${zmax}nm.dat\
                  -b ${begin}\
                  -e ${end}\
                  -dt ${dt}\
                  -bin ${bin_width}\
...
saveDir="densmap-z_Li_${zmin}-${zmax}nm_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_densmap-z_Li_${zmin}-${zmax}nm.dat\
          ${settings}_${system}_densmap-z_Li_${zmin}-${zmax}nm_slurm-${SLURM_JOB_ID}.out\
          ${saveDir}\
        || exit
