...
#SBATCH --job-name="gmx_msd_lateral-z"
#SBATCH --output="gmx_msd_lateral-z_slurm-%j.out"
#SBATCH --nodes=1
...
save_dir="msd_lateral-z_slurm-${SLURM_JOB_ID}"
if [[ ! -d ${save_dir} ]]; then
    echo -e "\n"
    mkdir -v "${save_dir}" || exit
    mv -v \
        "${settings}_${system}_msd_lateral-z.xvg" \
