...
#SBATCH --job-name="msd_lateral-z"             # Name for the job allocation
#SBATCH --output="msd_lateral-z_slurm-%j.out"  # Filename to which standard output and error will be written
...
if [[ ${system} == *gra* ]]; then
    selection="2 3 4 30 24"  # li ntf2 <ether> <ETHER>_&_O* electrolyte
else
    selection="2 3 4 14 0"   # li ntf2 <ether> <ETHER>_&_O* System
fi
echo -e "\n\n\n"
echo "=================================================================="
echo ${selection}\
    | gmx msd -f ${settings}_out_${system}_pbc_whole_mol_nojump.xtc\
              ...
              -n ${system}.ndx\
              -o ${settings}_${system}_msd_lateral-z.xvg\
              -b ${begin}\
              -e ${end}\
              -lateral z\
              -ngroup 4\
...
saveDir="msd_lateral-z_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_msd_lateral-z.xvg\
          ${settings}_${system}_msd_lateral-z_slurm-${SLURM_JOB_ID}.out\
          ${saveDir}\
        || exit
