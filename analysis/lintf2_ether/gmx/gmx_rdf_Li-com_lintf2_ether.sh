...
#SBATCH --job-name="rdf_Li-com"             # Name for the job allocation
#SBATCH --output="rdf_Li-com_slurm-%j.out"  # Filename to which standard output and error will be written
...
echo -e "\n\n\n"
echo "${cation}-${anion} (COM), ${cation}-${solvent} (COM)"
echo
echo "=================================================================="
gmx rdf -f ${settings}_out_${system}_pbc_whole_mol.xtc\
        -s ${settings}_${system}.tpr\
        -o ${settings}_${system}_rdf_Li-com.xvg\
        -cn ${settings}_${system}_cnrdf_Li-com.xvg\
        -b ${begin}\
        -e ${end}\
        -dt ${dt}\
        -bin ${bin_width}\
        -ref "whole_res_com of resname ${cation}"\
...
saveDir="rdf_Li-com_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_rdf_Li-com.xvg\
          ${settings}_${system}_cnrdf_Li-com.xvg\
          ${settings}_${system}_rdf_Li-com_slurm-${SLURM_JOB_ID}.out\
          ${saveDir}\
        || exit
