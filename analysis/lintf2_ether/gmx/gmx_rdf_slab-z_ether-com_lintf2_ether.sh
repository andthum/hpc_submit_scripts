...
#SBATCH --job-name="rdf_slab-z_ether-com"             # Name for the job allocation
#SBATCH --output="rdf_slab-z_ether-com_slurm-%j.out"  # Filename to which standard output and error will be written
...
gmx rdf -f ${settings}_out_${system}_pbc_whole_mol.xtc\
        ...
        -bin ${bin_width}\
        -xy\
        -ref "whole_res_com of resname ${solvent} and (z >= ${zmin} and z < ${zmax})"\
        -sel "whole_res_com of resname ${solvent} and (z >= ${zmin} and z < ${zmax})"\
    || exit
echo "=================================================================="
...
saveDir="rdf_slab-z_ether-com_${zmin}-${zmax}nm_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_rdf_slab-z_ether-com_${zmin}-${zmax}nm.xvg\
