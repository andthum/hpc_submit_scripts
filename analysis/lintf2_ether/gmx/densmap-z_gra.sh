...
#SBATCH --job-name="gmx_densmap-z_gra"
#SBATCH --output="gmx_densmap-z_gra_slurm-%j.out"
#SBATCH --nodes=1
...
if [[ ${system} != *gra* ]]; then
    echo
    echo "Error: The system contains no electrodes."
    exit 1
fi
...
save_dir="densmap-z_gra_${zmin}-${zmax}nm_slurm-${SLURM_JOB_ID}"
if [[ ! -d ${save_dir} ]]; then
    echo -e "\n"
    mkdir -v "${save_dir}" || exit
    mv -v \
        "${settings}_${system}_densmap-z_gra_${zmin}-${zmax}nm.dat" \
