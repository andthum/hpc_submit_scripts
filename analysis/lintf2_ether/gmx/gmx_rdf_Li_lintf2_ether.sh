...
#SBATCH --job-name="rdf_Li"             # Name for the job allocation
#SBATCH --output="rdf_Li_slurm-%j.out"  # Filename to which standard output and error will be written
...
saveDir="rdf_Li_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_rdf_Li.xvg\
