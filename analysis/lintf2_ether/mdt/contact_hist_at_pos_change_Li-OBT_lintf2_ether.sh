...
#SBATCH --job-name="contact_hist_at_pos_change_Li-OBT"             # Name for the job allocation
#SBATCH --output="contact_hist_at_pos_change_Li-OBT_slurm-%j.out"  # Filename to which standard output and error will be written
...
${HOME}/Promotion/mdtools/env/bin/python3 -u\
    ${HOME}/Promotion/mdtools/scripts/structure/contact_hist_at_pos_change.py\
...
saveDir="contact_hist_at_pos_change_Li-OBT_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_contact_hist_at_pos_change_Li-OBT*\
          ${saveDir}\
        || exit
