...
#SBATCH --job-name="lifetime_autocorr_Li-OBT"             # Name for the job allocation
#SBATCH --output="lifetime_autocorr_Li-OBT_slurm-%j.out"  # Filename to which standard output and error will be written
...
${HOME}/Promotion/mdtools/env/bin/python3 -u\
    ${HOME}/Promotion/mdtools/scripts/dynamics/lifetime_autocorr_parallel.py\
        ...
        -o ${settings}_${system}_lifetime_autocorr_Li-OBT.txt\
        --ref "type Li"\
        --sel "type OBT"\
        -c ${cutoff}\
        --compound "atoms"\
...
saveDir="lifetime_autocorr_Li-OBT_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_lifetime_autocorr_Li-OBT.txt\
          ${settings}_${system}_lifetime_autocorr_Li-OBT_slurm-${SLURM_JOB_ID}.out\
          ${saveDir}\
        || exit
