...
#SBATCH --job-name="msd_layer_NTf2"             # Name for the job allocation
#SBATCH --output="msd_layer_NTf2_slurm-%j.out"  # Filename to which standard output and error will be written
...
${HOME}/Promotion/mdtools/env/bin/python3 -u\
    ${HOME}/Promotion/mdtools/scripts/dynamics/msd_layer_serial.py\
        -f ${settings}_out_${system}_pbc_whole_mol_nojump.xtc\
        -s ${settings}_${system}.tpr\
        -o ${settings}_${system}_NTf2\
        --sel "resname ntf2"\
        --com residues\
        -d z\
        --bins ${settings}_${system}_Ndens-z_Li_binsA.txt\
        -b ${begin}\
        -e ${end}\
        --every ${every}\
        --nblocks ${nblocks}\
...
saveDir="msd_layer_NTf2_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_NTf2_msd_layer.txt\
          ${settings}_${system}_NTf2_m*dx_layer.txt\
          ${settings}_${system}_NTf2_m*dy_layer.txt\
          ${settings}_${system}_NTf2_m*dz_layer.txt\
          ${settings}_${system}_msd_layer_NTf2_slurm-${SLURM_JOB_ID}.out\
          ${saveDir}\
        || exit
