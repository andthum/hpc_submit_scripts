...
#SBATCH --job-name="msd_at_coord_change_Li-ether"             # Name for the job allocation
#SBATCH --output="msd_at_coord_change_Li-ether_slurm-%j.out"  # Filename to which standard output and error will be written
...
#SBATCH --mem=4G                  # Required memory per node
...
${HOME}/Promotion/mdtools/env/bin/python3 -u\
    ${HOME}/Promotion/mdtools/scripts/dynamics/msd_at_coord_change.py\
        -f ${settings}_out_${system}_pbc_whole_mol_nojump.xtc\
        -s ${settings}_${system}.tpr\
        -o ${settings}_${system}_msd_at_coord_change_Li-ether.txt\
        --ref "type Li"\
        --sel "type OE"\
        -c ${cutoff}\
        --compound "residues"\
...
saveDir="msd_at_coord_change_Li-ether_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_msd_at_coord_change_Li-ether.txt\
          ${settings}_${system}_msd_at_coord_change_Li-ether_slurm-${SLURM_JOB_ID}.out\
          ${saveDir}\
        || exit
