...
#SBATCH --job-name="msd_NTf2"             # Name for the job allocation
#SBATCH --output="msd_NTf2_slurm-%j.out"  # Filename to which standard output and error will be written
...
${HOME}/Promotion/mdtools/env/bin/python3 -u\
    ${HOME}/Promotion/mdtools/scripts/dynamics/msd_serial.py\
        ...
        -o ${settings}_${system}_msd_NTf2.txt\
        --sel "resname ntf2"\
        --com residues\
        -b ${begin}\
        -e ${end}\
...
saveDir="msd_NTf2_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_msd_NTf2.txt\
          ${settings}_${system}_msd_NTf2_slurm-${SLURM_JOB_ID}.out\
          ${saveDir}\
        || exit
