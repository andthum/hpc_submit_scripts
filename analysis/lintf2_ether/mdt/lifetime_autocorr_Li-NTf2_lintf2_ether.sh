...
#SBATCH --job-name="lifetime_autocorr_Li-NTf2"             # Name for the job allocation
#SBATCH --output="lifetime_autocorr_Li-NTf2_slurm-%j.out"  # Filename to which standard output and error will be written
...
#SBATCH --mem=6G                  # Required memory per node
...
${HOME}/Promotion/mdtools/env/bin/python3 -u\
    ${HOME}/Promotion/mdtools/scripts/dynamics/lifetime_autocorr_parallel.py\
        -f ${settings}_out_${system}_pbc_whole_mol.xtc\
        -s ${settings}_${system}.tpr\
        -o ${settings}_${system}_lifetime_autocorr_Li-NTf2.txt\
        --ref "type Li"\
        --sel "type OBT"\
        -c ${cutoff}\
        --compound "residues"\
...
saveDir="lifetime_autocorr_Li-NTf2_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_lifetime_autocorr_Li-NTf2.txt\
          ${settings}_${system}_lifetime_autocorr_Li-NTf2_slurm-${SLURM_JOB_ID}.out\
          ${saveDir}\
        || exit
