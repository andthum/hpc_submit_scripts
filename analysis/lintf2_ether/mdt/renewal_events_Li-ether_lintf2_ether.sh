...
#SBATCH --job-name="renewal_events_Li-ether"             # Name for the job allocation
#SBATCH --output="renewal_events_Li-ether_slurm-%j.out"  # Filename to which standard output and error will be written
...
if [ -z ${intermittency} ]; then
    intermittency=0
    echo
    echo "intermittency is not set. Going to set it to 0"
    echo "intermittency = ${intermittency}"
fi
...
${HOME}/Promotion/mdtools/env/bin/python3 -u\
    ${HOME}/Promotion/mdtools/scripts/dynamics/extract_renewal_events.py\
        -f ${settings}_out_${system}_pbc_whole_mol_nojump.xtc\
        -s ${settings}_${system}.tpr\
        -o ${settings}_${system}_renewal_events_Li-ether.txt\
        --dtrj ${settings}_${system}_renewal_events_Li-ether.npy\
        --ref "type Li"\
        --sel "type OE"\
        -c ${cutoff}\
        --compound "residues"\
...
saveDir="renewal_events_Li-ether_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_renewal_events_Li-ether.txt\
          ${settings}_${system}_renewal_events_Li-ether.npy\
          ${settings}_${system}_renewal_events_Li-ether_slurm-${SLURM_JOB_ID}.out\
          ${saveDir}\
        || exit
