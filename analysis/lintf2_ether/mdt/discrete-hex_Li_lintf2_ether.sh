...
#SBATCH --job-name="discrete-hex_Li"             # Name for the job allocation
#SBATCH --output="discrete-hex_Li_slurm-%j.out"  # Filename to which standard output and error will be written
...
zmin=${6}         # Only atoms in a slab in the xy-plane between zmin
zmax=${7}         # and zmax are considered (in Angstrom)
...
if [ -z ${zmax} ]; then
    zmax=$(echo "${zmin} + 10.0" | bc || exit)
    zmax=$(printf "%.1f" "${zmax}" || exit)
    echo
    echo "zmax is not set. Going to set it to zmin + 10.0"
    echo "zmax = ${zmax}"
fi

if (( $(echo "${zmax} > 51.39" | bc || exit) )); then
    electrode="graB"
else
    electrode="graT"
fi
...
${HOME}/Promotion/mdtools/env/bin/python3 -u\
    ${HOME}/Promotion/mdtools/scripts/structure/discrete_hex.py\
...
saveDir="discrete-hex_Li_${zmin}-${zmax}A_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_discrete-hex_Li_${zmin}-${zmax}A*\
          ${saveDir}\
        || exit
