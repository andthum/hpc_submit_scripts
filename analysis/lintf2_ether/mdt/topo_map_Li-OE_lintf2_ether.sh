...
#SBATCH --job-name="topo_map_Li-OE"             # Name for the job allocation
#SBATCH --output="topo_map_Li-OE_slurm-%j.out"  # Filename to which standard output and error will be written
...
${HOME}/Promotion/mdtools/env/bin/python3 -u\
    ${HOME}/Promotion/mdtools/scripts/structure/topological_map.py\
        ...
        -o ${settings}_${system}_topo_map_Li-OE_${li_ix}.txt\
        --ref "resid ${li_ix}"\
        --sel "type OE"\
        -c ${cutoff}\
        -b ${begin}\
...
saveDir="topo_map_Li-OE_${li_ix}_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_topo_map_Li-OE_${li_ix}.txt\
          ${settings}_${system}_topo_map_Li-OE_${li_ix}_slurm-${SLURM_JOB_ID}.out\
          ${saveDir}\
        || exit
