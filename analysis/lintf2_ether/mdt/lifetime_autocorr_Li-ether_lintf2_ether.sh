...
#SBATCH --job-name="lifetime_autocorr_Li-ether"             # Name for the job allocation
#SBATCH --output="lifetime_autocorr_Li-ether_slurm-%j.out"  # Filename to which standard output and error will be written
...
saveDir="lifetime_autocorr_Li-ether_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_lifetime_autocorr_Li-ether.txt\
