...
#SBATCH --job-name="mdt_lig_change_at_pos_change_blocks_hist_Li-OBT"
...
${py_exe} -u \
    ...
    -o "${settings}_${system}_lig_change_at_pos_change_blocks_hist_Li-OBT.txt" \
    -b "${begin}" \
    -e "${end}" \
    --every "${every}" \
    --ref "type Li" \
    ...
    --bins-out "${settings}_${system}_lig_change_at_pos_change_blocks_hist_Li-OBT_bins.txt" ||
    exit
echo "=================================================================="
...
save_dir="lig_change_at_pos_change_blocks_hist_Li-OBT_slurm-${SLURM_JOB_ID}"
if [[ ! -d ${save_dir} ]]; then
    echo -e "\n"
    mkdir -v "${save_dir}" || exit
    mv -v \
        "${settings}_${system}_lig_change_at_pos_change_blocks_hist_Li-OBT.txt" \
        "${settings}_${system}_lig_change_at_pos_change_blocks_hist_Li-OBT_bins.txt" \
        "${settings}_${system}_lig_change_at_pos_change_blocks_hist_Li-OBT_slurm-${SLURM_JOB_ID}.out" \
        "${save_dir}"
