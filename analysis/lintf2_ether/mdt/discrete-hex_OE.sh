...
#SBATCH --job-name="mdt_discrete-hex_OE"
#SBATCH --output="mdt_discrete-hex_OE_slurm-%j.out"
#SBATCH --nodes=1
...
${py_exe} -u \
    ...
    -o "${settings}_${system}_discrete-hex_OE_${zmin}-${zmax}A" \
    -b "${begin}" \
    -e "${end}" \
    --every "${every}" \
    --sel "type OE" \
...
if [[ -f ${settings}_${system}_discrete-hex_OE_${zmin}-${zmax}A_traj.npy ]]; then
    echo -e "\n"
    mv -v \
        "${settings}_${system}_discrete-hex_OE_${zmin}-${zmax}A_traj.npy" \
        "${settings}_${system}_discrete-hex_OE_${zmin}-${zmax}A_dtrj.npy"
fi
...
${py_exe} -u \
    ...
    -f "${settings}_${system}_discrete-hex_OE_${zmin}-${zmax}A_dtrj.npy" \
    -o "${settings}_${system}_discrete-hex_OE_${zmin}-${zmax}A_state_lifetime_discard-all-neg_continuous.txt" \
    -b "0" \
    -e "-1" \
    --every "1" \
    --nblocks "${nblocks}" \
...
${py_exe} -u \
    "${mdt_path}/scripts/discretization/state_lifetime.py" \
    -f "${settings}_${system}_discrete-hex_OE_${zmin}-${zmax}A_dtrj.npy" \
    -o "${settings}_${system}_discrete-hex_OE_${zmin}-${zmax}A_state_lifetime_discard-all-neg.txt" \
    -b "0" \
    -e "-1" \
    --every "1" \
    --nblocks "${nblocks}" \
...
save_dir="discrete-hex_OE_${zmin}-${zmax}A_slurm-${SLURM_JOB_ID}"
if [[ ! -d ${save_dir} ]]; then
    echo -e "\n"
    mkdir -v "${save_dir}" || exit
    mv -v \
        "${settings}_${system}_discrete-hex_OE_${zmin}-${zmax}A_lattice_faces.npy" \
        "${settings}_${system}_discrete-hex_OE_${zmin}-${zmax}A_lattice_vertices.npy" \
        "${settings}_${system}_discrete-hex_OE_${zmin}-${zmax}A_dtrj.npy" \
        "${settings}_${system}_discrete-hex_OE_${zmin}-${zmax}A_slurm-${SLURM_JOB_ID}.out" \
        "${settings}_${system}_discrete-hex_OE_${zmin}-${zmax}A_state_lifetime_discard-all-neg.txt" \
        "${settings}_${system}_discrete-hex_OE_${zmin}-${zmax}A_state_lifetime_discard-all-neg_continuous.txt" \
        "${save_dir}"
