...
${py_exe} -u \
    ...
    --dtrj "${settings}_${system}_renewal_events_Li-ether_dtrj.npy" \
    -b "${begin}" \
    -e "${end}" \
    --every "${every}" \
    --ref "type Li" \
...
save_dir="renewal_events_Li-ether_slurm-${SLURM_JOB_ID}"
if [[ ! -d ${save_dir} ]]; then
    echo -e "\n"
    mkdir -v "${save_dir}" || exit
    mv -v \
        "${settings}_${system}_renewal_events_Li-ether.txt" \
