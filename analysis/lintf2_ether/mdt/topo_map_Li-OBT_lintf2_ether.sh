...
#SBATCH --job-name="topo_map_Li-OBT"             # Name for the job allocation
#SBATCH --output="topo_map_Li-OBT_slurm-%j.out"  # Filename to which standard output and error will be written
...
li_ix=${7}        # Index of the lithium ion to analyze
...
if [ -z ${li_ix} ]; then
    li_ix=0
    echo
    echo "li_ix is not set. Going to set it to 0"
    echo "li_ix = ${li_ix}"
fi
...
${HOME}/Promotion/mdtools/env/bin/python3 -u\
    ${HOME}/Promotion/mdtools/scripts/structure/topological_map.py\
        -f ${settings}_out_${system}_pbc_whole_mol.xtc\
        -s ${settings}_${system}.tpr\
        -o ${settings}_${system}_topo_map_Li-OBT_${li_ix}.txt\
        --ref "resid ${li_ix}"\
        --sel "type OBT"\
        -c ${cutoff}\
        -b ${begin}\
...
saveDir="topo_map_Li-OBT_${li_ix}_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_topo_map_Li-OBT_${li_ix}.txt\
          ${settings}_${system}_topo_map_Li-OBT_${li_ix}_slurm-${SLURM_JOB_ID}.out*\
          ${saveDir}\
        || exit
