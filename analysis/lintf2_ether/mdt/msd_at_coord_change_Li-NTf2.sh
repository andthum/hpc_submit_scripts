...
#SBATCH --job-name="mdt_msd_at_coord_change_Li-NTf2"
#SBATCH --output="mdt_msd_at_coord_change_Li-NTf2_slurm-%j.out"
#SBATCH --nodes=1
...
save_dir="msd_at_coord_change_Li-NTf2_slurm-${SLURM_JOB_ID}"
if [[ ! -d ${save_dir} ]]; then
    echo -e "\n"
    mkdir -v "${save_dir}" || exit
    mv -v \
        "${settings}_${system}_msd_at_coord_change_Li-NTf2.txt" \
