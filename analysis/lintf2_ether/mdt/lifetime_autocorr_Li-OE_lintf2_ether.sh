...
#SBATCH --job-name="lifetime_autocorr_Li-OE"             # Name for the job allocation
#SBATCH --output="lifetime_autocorr_Li-OE_slurm-%j.out"  # Filename to which standard output and error will be written
...
#SBATCH --mem=12G                 # Required memory per node
...
${HOME}/Promotion/mdtools/env/bin/python3 -u\
    ${HOME}/Promotion/mdtools/scripts/dynamics/lifetime_autocorr_parallel.py\
        ...
        -o ${settings}_${system}_lifetime_autocorr_Li-OE.txt\
        --ref "type Li"\
        --sel "type OE"\
        -c ${cutoff}\
        --compound "atoms"\
...
saveDir="lifetime_autocorr_Li-OE_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_lifetime_autocorr_Li-OE.txt\
          ${settings}_${system}_lifetime_autocorr_Li-OE_slurm-${SLURM_JOB_ID}.out\
          ${saveDir}\
        || exit
