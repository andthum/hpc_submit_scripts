...
#SBATCH --job-name="mdt_axial_hex_dist_2nn_NBT"
#SBATCH --output="mdt_axial_hex_dist_2nn_NBT_slurm-%j.out"
#SBATCH --nodes=1
...
${py_exe} -u \
    ...
    -o "${settings}_${system}_axial_hex_dist_2nn_NBT_${zmin}-${zmax}A.txt" \
    -b "${begin}" \
    -e "${end}" \
    --every "${every}" \
    --sel "type NBT" \
...
save_dir="axial_hex_dist_2nn_NBT_${zmin}-${zmax}A_slurm-${SLURM_JOB_ID}"
if [[ ! -d ${save_dir} ]]; then
    echo -e "\n"
    mkdir -v "${save_dir}" || exit
    mv -v \
        "${settings}_${system}_axial_hex_dist_2nn_NBT_${zmin}-${zmax}A.txt" \
        "${settings}_${system}_axial_hex_dist_2nn_NBT_${zmin}-${zmax}A_slurm-${SLURM_JOB_ID}.out" \
        "${save_dir}"
