...
#SBATCH --job-name="msd_layer_Li"             # Name for the job allocation
#SBATCH --output="msd_layer_Li_slurm-%j.out"  # Filename to which standard output and error will be written
...
saveDir="msd_layer_Li_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_Li_msd_layer.txt\
          ${settings}_${system}_Li_m*dx_layer.txt\
          ${settings}_${system}_Li_m*dy_layer.txt\
          ${settings}_${system}_Li_m*dz_layer.txt\
          ${settings}_${system}_msd_layer_Li_slurm-${SLURM_JOB_ID}.out\
          ${saveDir}\
        || exit
