...
#SBATCH --job-name="mdt_msd_NBT"
#SBATCH --output="mdt_msd_NBT_slurm-%j.out"
#SBATCH --nodes=1
...
${py_exe} -u \
    ...
    -o "${settings}_${system}_msd_NBT.txt" \
    -b "${begin}" \
    -e "${end}" \
    --every "${every}" \
    --nblocks "${nblocks}" \
...
save_dir="msd_NBT_slurm-${SLURM_JOB_ID}"
if [[ ! -d ${save_dir} ]]; then
    echo -e "\n"
    mkdir -v "${save_dir}" || exit
    mv -v \
        "${settings}_${system}_msd_NBT.txt" \
        "${settings}_${system}_msd_NBT_slurm-${SLURM_JOB_ID}.out" \
        "${save_dir}"
