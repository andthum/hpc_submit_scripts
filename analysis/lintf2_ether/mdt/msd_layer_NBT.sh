...
#SBATCH --job-name="mdt_msd_layer_NBT"
#SBATCH --output="mdt_msd_layer_NBT_slurm-%j.out"
#SBATCH --nodes=1
...
gzip --best --verbose "${settings}_${system}_NBT_msd_layer.txt" || exit
gzip --best --verbose "${settings}_${system}_NBT_msdx_layer.txt" || exit
gzip --best --verbose "${settings}_${system}_NBT_msdy_layer.txt" || exit
gzip --best --verbose "${settings}_${system}_NBT_msdz_layer.txt" || exit
gzip --best --verbose "${settings}_${system}_NBT_mdx_layer.txt" || exit
gzip --best --verbose "${settings}_${system}_NBT_mdy_layer.txt" || exit
gzip --best --verbose "${settings}_${system}_NBT_mdz_layer.txt" || exit
...
if [[ ! -d ${save_dir} ]]; then
    ...
    mv -v \
        "${settings}_${system}_NBT_msd_layer.txt.gz" \
        "${settings}_${system}_NBT_msdx_layer.txt.gz" \
        "${settings}_${system}_NBT_msdy_layer.txt.gz" \
        "${settings}_${system}_NBT_msdz_layer.txt.gz" \
        "${settings}_${system}_NBT_mdx_layer.txt.gz" \
        "${settings}_${system}_NBT_mdy_layer.txt.gz" \
        "${settings}_${system}_NBT_mdz_layer.txt.gz" \
