...
#SBATCH --job-name="mdt_lig_change_at_pos_change_blocks_hist_Li-OE"
#SBATCH --output="mdt_lig_change_at_pos_change_blocks_hist_Li-OE_slurm-%j.out"
#SBATCH --nodes=1
...
${py_exe} -u \
    ...
    -o "${settings}_${system}_lig_change_at_pos_change_blocks_hist_Li-OE.txt" \
    -b "${begin}" \
    -e "${end}" \
    --every "${every}" \
    --ref "type Li" \
    ...
    --bins-out "${settings}_${system}_lig_change_at_pos_change_blocks_hist_Li-OE_bins.txt" ||
    exit
echo "=================================================================="
...
save_dir="lig_change_at_pos_change_blocks_hist_Li-OE_slurm-${SLURM_JOB_ID}"
if [[ ! -d ${save_dir} ]]; then
    echo -e "\n"
    mkdir -v "${save_dir}" || exit
    mv -v \
        "${settings}_${system}_lig_change_at_pos_change_blocks_hist_Li-OE.txt" \
        "${settings}_${system}_lig_change_at_pos_change_blocks_hist_Li-OE_bins.txt" \
        "${settings}_${system}_lig_change_at_pos_change_blocks_hist_Li-OE_slurm-${SLURM_JOB_ID}.out" \
        "${save_dir}"
