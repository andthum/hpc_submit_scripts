...
#SBATCH --job-name="contact_hist_Li-OE"             # Name for the job allocation
#SBATCH --output="contact_hist_Li-OE_slurm-%j.out"  # Filename to which standard output and error will be written
...
${HOME}/Promotion/mdtools/env/bin/python3 -u\
    ${HOME}/Promotion/mdtools/scripts/structure/contact_hist.py\
        ...
        -o ${settings}_${system}_contact_hist_Li-OE.txt\
        -b ${begin}\
        -e ${end}\
        --every ${every}\
        --ref "type Li"\
...
saveDir="contact_hist_Li-OE_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_contact_hist_Li-OE.txt\
          ${settings}_${system}_contact_hist_Li-OE_slurm-${SLURM_JOB_ID}.out\
          ${saveDir}\
        || exit
