...
#SBATCH --job-name="mdt_msd_layer_NTf2"
#SBATCH --output="mdt_msd_layer_NTf2_slurm-%j.out"
#SBATCH --nodes=1
...
save_dir="msd_layer_NTf2_slurm-${SLURM_JOB_ID}"
if [[ ! -d ${save_dir} ]]; then
    echo -e "\n"
    ...
    mv -v \
        ...
        "${settings}_${system}_NTf2_m"?(s)"dx_layer.txt" \
        "${settings}_${system}_NTf2_m"?(s)"dy_layer.txt" \
        "${settings}_${system}_NTf2_m"?(s)"dz_layer.txt" \
