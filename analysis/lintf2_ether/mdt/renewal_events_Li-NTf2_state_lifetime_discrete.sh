...
${py_exe} -u \
    ...
    -o "${settings}_${system}_renewal_events_Li-NTf2_state_lifetime_discrete_discard-neg-start_continuous.txt" \
    -b "0" \
    -e "-1" \
    --every "1" \
    --restart "${restart}" \
...
${py_exe} -u \
    ...
    --f1 "${settings}_${system}_renewal_events_Li-NTf2_dtrj.npy" \
    --f2 "${settings}_${system}_discrete-z_Li_dtrj.npy" \
    -o "${settings}_${system}_renewal_events_Li-NTf2_state_lifetime_discrete_discard-neg-start.txt" \
    -b "0" \
    -e "-1" \
    --every "1" \
    --restart "${restart}" \
...
save_dir="renewal_events_Li-NTf2_state_lifetime_discrete_slurm-${SLURM_JOB_ID}"
if [[ ! -d ${save_dir} ]]; then
    echo -e "\n"
    mkdir -v "${save_dir}" || exit
    mv -v \
        "${settings}_${system}_renewal_events_Li-NTf2_state_lifetime_discrete_discard-neg-start.txt" \
        "${settings}_${system}_renewal_events_Li-NTf2_state_lifetime_discrete_discard-neg-start_continuous.txt" \
        "${settings}_${system}_renewal_events_Li-NTf2_state_lifetime_discrete_slurm-${SLURM_JOB_ID}.out" \
        "${save_dir}"
