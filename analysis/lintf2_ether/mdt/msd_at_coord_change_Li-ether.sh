...
#SBATCH --job-name="mdt_msd_at_coord_change_Li-ether"
#SBATCH --output="mdt_msd_at_coord_change_Li-ether_slurm-%j.out"
#SBATCH --nodes=1
...
echo "================================================================="
${py_exe} -u \
    "${mdt_path}/scripts/dynamics/msd_at_coord_change.py" \
...
save_dir="msd_at_coord_change_Li-ether_slurm-${SLURM_JOB_ID}"
if [[ ! -d ${save_dir} ]]; then
    echo -e "\n"
    mkdir -v "${save_dir}" || exit
    mv -v \
        "${settings}_${system}_msd_at_coord_change_Li-ether.txt" \
