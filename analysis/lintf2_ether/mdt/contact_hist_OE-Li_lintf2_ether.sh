...
#SBATCH --job-name="contact_hist_OE-Li"             # Name for the job allocation
#SBATCH --output="contact_hist_OE-Li_slurm-%j.out"  # Filename to which standard output and error will be written
...
saveDir="contact_hist_OE-Li_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_contact_hist_OE-Li.txt\
