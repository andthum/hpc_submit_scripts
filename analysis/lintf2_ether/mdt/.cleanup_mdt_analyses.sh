...
gather_cmps() {
    ...
    mkdir "${root}/" || exit
    for cmp in ${cmps}; do
        if [[ -n $(find . -maxdepth 1 -type d -name "${root}_${cmp}_*slurm-[0-9]*" -print -quit) ]]; then
            mv "${root}_${cmp}"_*slurm-[0-9]*/ "${root}/" || exit
        fi
    done

    # Check if the root directory is empty.
    if [[ -n $(find "${root}/" -prune -empty) ]]; then
        echo "WARNING: No directory matching pattern '${root}_<compound>_*slurm-[0-9]*'"
        rm -r "${root:?}/" || exit
        return 0
    fi
...
gather_cmps "msd_layer" "Li NBT NTf2 OBT OE NTf2 ether"
