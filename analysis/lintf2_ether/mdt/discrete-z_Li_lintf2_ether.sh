...
#SBATCH --job-name="discrete-z_Li"             # Name for the job allocation
#SBATCH --output="discrete-z_Li_slurm-%j.out"  # Filename to which standard output and error will be written
...
${HOME}/Promotion/mdtools/env/bin/python3 -u\
    ${HOME}/Promotion/mdtools/scripts/discretization/discrete_pos.py\
        -f ${settings}_out_${system}_pbc_whole_mol.xtc\
        -s ${settings}_${system}.tpr\
        -o ${settings}_${system}_discrete-z_Li\
        -b ${begin}\
        -e ${end}\
        --every ${every}\
        --sel "type Li"\
...
saveDir="discrete-z_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_discrete-z_Li*\
          ${saveDir}\
        || exit
