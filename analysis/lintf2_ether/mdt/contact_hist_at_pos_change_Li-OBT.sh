...
#SBATCH --job-name="mdt_contact_hist_at_pos_change_Li-OBT"
#SBATCH --output="mdt_contact_hist_at_pos_change_Li-OBT_slurm-%j.out"
#SBATCH --nodes=1
...
${py_exe} -u \
    ...
    -o "${settings}_${system}_contact_hist_at_pos_change_Li-OBT" \
    -b "${begin}" \
    -e "${end}" \
    --every "${every}" \
    --ref "type Li" \
...
save_dir="contact_hist_at_pos_change_Li-OBT_slurm-${SLURM_JOB_ID}"
if [[ ! -d ${save_dir} ]]; then
    echo -e "\n"
    mkdir -v "${save_dir}" || exit
    mv -v \
        "${settings}_${system}_contact_hist_at_pos_change_Li-OBT_stay.txt" \
        "${settings}_${system}_contact_hist_at_pos_change_Li-OBT_leave.txt" \
        "${settings}_${system}_contact_hist_at_pos_change_Li-OBT_bins.txt" \
        "${settings}_${system}_contact_hist_at_pos_change_Li-OBT_slurm-${SLURM_JOB_ID}.out" \
        "${save_dir}"
