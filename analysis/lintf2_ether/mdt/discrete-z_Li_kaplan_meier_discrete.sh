...
#SBATCH --job-name="mdt_discrete-z_Li_kaplan_meier_discrete"
#SBATCH --output="mdt_discrete-z_Li_kaplan_meier_discrete_slurm-%j.out"
#SBATCH --nodes=1
...
analysis="discrete-z_Li_kaplan_meier_discrete"
...
${py_exe} -u \
    ...
    --f1 "${infile}" \
    --f2 "${infile}" \
    --o-sf "${settings}_${system}_${analysis}_sf.txt.gz" \
    --o-var "${settings}_${system}_${analysis}_sf_var.txt.gz" \
    -b "0" \
    -e "-1" \
    --every "1" ||
    exit
...
if [[ ! -d ${save_dir} ]]; then
    ...
    mv -v \
        "${settings}_${system}_${analysis}_sf.txt.gz" \
        "${settings}_${system}_${analysis}_sf_var.txt.gz" \
