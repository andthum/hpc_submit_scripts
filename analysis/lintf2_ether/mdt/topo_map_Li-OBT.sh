...
#SBATCH --job-name="mdt_topo_map_Li-OBT"
#SBATCH --output="mdt_topo_map_Li-OBT_slurm-%j.out"
#SBATCH --nodes=1
...
li_ix=${11}   # Index of the lithium ion to analyze
...
${py_exe} -u \
    ...
    -o "${settings}_${system}_topo_map_Li-OBT_Li${li_ix}.txt" \
    -b "${begin}" \
    -e "${end}" \
    --every "${every}" \
    --ref "index ${li_ix}" \
...
save_dir="topo_map_Li-OBT_Li${li_ix}_slurm-${SLURM_JOB_ID}"
if [[ ! -d ${save_dir} ]]; then
    echo -e "\n"
    mkdir -v "${save_dir}" || exit
    mv -v \
        "${settings}_${system}_topo_map_Li-OBT_Li${li_ix}.txt" \
        "${settings}_${system}_topo_map_Li-OBT_slurm-${SLURM_JOB_ID}.out" \
        "${save_dir}"
