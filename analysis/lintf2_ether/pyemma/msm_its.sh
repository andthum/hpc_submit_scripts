...
#SBATCH --job-name="msm_its"             # Name for the job allocation
#SBATCH --output="msm_its_slurm-%j.out"  # Filename to which standard output and error will be written
...
error=${6}        # Whether to estimate the uncertainty of the model or not
...
if [[ ${error} == true ]]; then
    ${HOME}/Promotion/mdtools/env/bin/python3 -u\
        ${HOME}/Promotion/mdtools/scripts/markov_modeling/pyemma_its.py\
            -f ${settings}_${system}_msm-${direction}_${residue}_bin${num_bins}_discrete_traj.npy\
            -o ${settings}_${system}_msm-${direction}_${residue}_bin${num_bins}\
            --no-plots\
            --errors\
            --nits 16\
            --nsamples 4\
...
saveDir="msm-${direction}_${residue}_bin${num_bins}_its_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_msm-${direction}_${residue}_bin${num_bins}_its*\
          ${saveDir}\
        || exit
