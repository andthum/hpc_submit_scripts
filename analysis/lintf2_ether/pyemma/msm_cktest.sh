...
#SBATCH --job-name="msm_cktest_pyemma"             # Name for the job allocation
#SBATCH --output="msm_cktest_pyemma_slurm-%j.out"  # Filename to which standard output and error will be written
...
${HOME}/Promotion/mdtools/env/bin/python3 -u\
    ${HOME}/Promotion/mdtools/scripts/markov_modeling/pyemma_cktest.py\
        -f ${settings}_${system}_msm-${direction}_${residue}_bin${num_bins}_lag${lagtime}_mm.h5\
...
saveDir="msm-${direction}_${residue}_bin${num_bins}_lag${lagtime}_cktest_slurm-${SLURM_JOB_ID}"
parrentSaveDir="ana_${settings}_${system}"
if [ ! -d ${saveDir} ]; then
    echo -e "\n\n\n"
    mkdir -v ${saveDir} || exit
    mv -v ${settings}_${system}_msm-${direction}_${residue}_bin${num_bins}_lag${lagtime}_cktest*\
          ${saveDir}\
        || exit
