...
#SBATCH --job-name="msm_discrete"             # Name for the job allocation
#SBATCH --output="msm_discrete_slurm-%j.out"  # Filename to which standard output and error will be written
...
begin=${4}        # First frame to read. Frame numbering starts at 0
end=${5}          # Last frame to read (exclusive)
every=${6}        # Read every n-th frame
direction=${7}    # Spatial direction to discretize
...
if [[ ${system} == *_gra_* ]] && [[ ${direction} == z ]]; then
    PBC=false
else
    PBC=true
fi
...
if [[ ${residue} == li ]]; then
    if [[ ${PBC} == false ]]; then
        ${HOME}/Promotion/mdtools/env/bin/python3 -u\
            ${HOME}/Promotion/mdtools/scripts/markov_modeling/discrete_pos.py\
                -f ${settings}_out_${system}_pbc_whole_mol.xtc\
                -s ${settings}_${system}.tpr\
                -o ${settings}_${system}_msm-${direction}_${residue}_bin${num_bins}_discrete\
                --no-plots\
                --sel "resname ${residue}"\
                -b ${begin}\
                -e ${end}\
...
# Ouput files are needed by pyemma_its.py and pyemma_mm.py => do not
# rename them them or move them to another directory

# saveDir="msm-${direction}_${residue}_bin${num_bins}_discrete_slurm-${SLURM_JOB_ID}"
# parrentSaveDir="ana_${settings}_${system}"
# if [ ! -d ${saveDir} ]; then
#     echo -e "\n\n\n"
#     mkdir -v ${saveDir} || exit
#     mv -v ${settings}_${system}_msm-${direction}_${residue}_bin${num_bins}_discrete*\
#           ${saveDir}\
#         || exit
